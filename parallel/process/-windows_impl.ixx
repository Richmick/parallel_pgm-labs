...
namespace parallel
{
	namespace process
	{
		...
		{
			static_assert((ControlPol == control_policy::anonymous_tube)
						|| (ControlPol == control_policy::named_tube), "not implemented");

		private:
			using process_info = windows_pipe_data;

			static constexpr inline bool has_pipe = (ControlPol == control_policy::anonymous_tube)
					|| (ControlPol == control_policy::named_tube);
